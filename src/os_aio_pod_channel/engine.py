...
class Engine(Server):
    async def on_stop(self):
        ...
        await self.channel_manager.close(
            timeout=self.config.close_wait, now=self.context.loop.time()
        )
        self.stopping = False
        self.stopped = True
    ...
    def force_close_endpoint(self, endpoint):
        ...
        self.logger.warn(f"Not allowed new connection {info}")
        try:
            endpoint.close()
        except Exception as e:
            self.logger.error(f"Close endpoint error {info} {e}")
